...
veeamDashboardUrl=$(curl -X GET --header "Accept:application/json" --header "Authorization:Bearer $veeamBearer" "$veeamVANUrl" 2>&1 -k --silent)  
  ...
  curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_version,vanhostname=$veeamAHVBackuphostname,vanip=$veeamAHVBackupIP,vandescription=$veeamAHVBackupdescription,vanoperation=$veeamAHVBackupoperationMode,vanversion=$veeamAHVBackupversion,vanname=$veeamAHVBackupname vanbuild=$veeamAHVBackupbuild"
##
...
veeamDashboardUrl=$(curl -X GET --header "Accept:application/json" --header "Authorization:Bearer $veeamBearer" "$veeamVANUrl" 2>&1 -k --silent)
  ...
  clusterAvaliability=$(echo "$veeamDashboardUrl" | jq --raw-output ".clusterAvaliability") 
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_dashboard,veeamahvId=$veeamAHVBackupServerId totalVMsCount=$totalVMsCount,protectedVmsCount=$protectedVmsCount,protectedVmsWithSnapshotsCount=$protectedVmsWithSnapshotsCount,notProtectedVmsCount=$notProtectedVmsCount,totalJobsCount=$totalJobsCount,totalRepositoriesCount=$totalRepositoriesCount,repositoriesOutOfSpaceCount=$repositoriesOutOfSpaceCount,repositoriesLowOnSpaceCount=$repositoriesLowOnSpaceCount,backupServerAvaliability=$backupavailable,clusterAvaliability=$clusteravailable"
...
for id in $(echo "$veeamBackupUrl" | jq -r '.[].policyId'); do
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_sessions,policyname=$veeamPolicyName veeamBackupSessionsJobResult=$jobStatus,bytessecond=$veeamPolicyBytesSecond,bytestransferred=$veeamPolicyTransferred,bytesprocessed=$veeamPolicyProcessed,bytesread=$veeamPolicyRead,duration=$veeamPolicyDuration $creationTimeUnix"
...
for id in $(echo "$veeamBackupUrl" | jq -r '.Members[]."@odata.id"'); do
   ...
   #echo "veeam_nutanix_vms,vmname=$veeamVMName,vmcluster=$veeamVMCluster vmsnapshots=$veeamVMSnapshots,vmbackups=$veeamVMBackups $lastprotectionUnix"
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_vms,vmname=$veeamVMName,vmcluster=$veeamVMCluster vmsnapshots=$veeamVMSnapshots,vmbackups=$veeamVMBackups $lastprotectionUnix"
    arrayvms=$arrayvms+1   
done    
...
for id in $(echo "$veeamBackupUrl" | jq -r '.[].id'); do
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_server,ServerId=$veeamBackupServerId,ServerStatus=$veeamBackupServerStatus,ServerVersion=$veeamBackupServerVersion,ServerDescription=$veeamBackupServerDescription ServerPort=$veeamBackupServerPort"
    ...
    curl -i -XPOST "$veeamInfluxDBURL:$veeamInfluxDBPort/api/v2/write?org=$veeamInfluxDBOrg&bucket=$veeamInfluxDBBucket&precision=s" -H "Authorization: Token $veeamInfluxDBToken" --data-binary "veeam_nutanix_repository,repoName=$veeamRepoName,repoID=$veeamRepoId repoFreeSpace=$veeamRepoFreeSpace,repoTotalSpace=$veeamRepoTotalSpace"
