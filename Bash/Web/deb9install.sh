...
# Description :
#  .    Install a deb9 machine with a webserver of choice with php7-fpm & mariadb-server
#  .
#
...
if [[ ! -f "/etc/issue" ]] && [[ $(cat /etc/issue) != "Debian GNU/Linux 9 \n \l" ]]; then
  log "This is not a Debian 9, sorry. Aborted..."
  exit 1
fi
...
while true; do
  ...
  case ${web_server} in
  apache2)
    # Install Apache to work with PHP-FPM
    apt-get install apache2 php7.0 php-fpm php7.0-gd php7.0-intl php7.0-xsl php7.0-mcrypt php7.0-memcached php7.0-curl curl imagemagick php7.0-imagick
...
<VirtualHost *:8888>
  ...
  <IfModule mod_fastcgi.c>
    ...
    Alias /php7.0-fcgi /opt/phpmyadmin/cgi-bin/php7.0-fcgi-*-80
    FastCgiExternalServer /opt/phpmyadmin/cgi-bin/php7.0-fcgi-*-80 -idle-timeout 300 -socket /opt/phpmyadmin/.socks/phpmyadmin.sock -pass-header Authorization
  </IfModule>
...
;;
  nginx)
    # Install Nginx
    apt-get install nginx-full php7.0 php7.0-gd php7.0-intl php7.0-xsl php7.0-mcrypt php7.0-memcached php7.0-fpm php7.0-curl curl imagemagick php7.0-imagick apache2-utils
...
" > /etc/php7.0/fpm/conf.d/custom.ini

cat >> /etc/php7.0/fpm/pool.d/pool.example << _EOF_
[oprod]
...
php_admin_value[open_basedir] = /home/\$pool/www:/home/\$pool/tmp:/home/\$pool/sessions:/usr/share/php7.0:/usr/share/php:/tmp:/home/solr
...
security.limit_extensions = .php .php7.0 .php3 .php4 .html .htm
_EOF_
...
## Install drush
echo -e "Do you want to install drush"
...
do
  case ${choice} in
    yes)
      ...
      ;;
    no)
      log "Drush will not be installed"
      break
      ;;
  esac
done
...
#### Configure OpCache ####
if [ -f /etc/php7.0/fpm/conf.d/05-opcache.ini ]
then
      cat >> /etc/php7.0/fpm/conf.d/05-opcache.ini << _EOF_
...
_EOF_
        ...
        echo "php7.0-fpm seems to be not installed "
fi
...
if [[ ! -f "/etc/php7.0/fpm/pool.d/phpmyadmin.conf" ]]; then
cat >> /etc/php7.0/fpm/pool.d/phpmyadmin.conf << __EOF__
[phpmyadmin]
...
security.limit_extensions = .php .php7.0 .html .htm
__EOF__
fi

a2ensite phpmyadmin && service apache2 restart && service php-fpm restart
...
clear
echo -e "Do you want to install mod_security ?"
select choix1 in "yes" "no"
do
    case ${choix1} in
        yes)
        echo "script d'install mod_security à faire"
        #service apache2 restart
...
#### Install Pureftpd ####
clear
echo -e "Do you want to install Pureftpd ?"
select choix3 in "yes" "no"
do
    case ${choix3} in
     yes)
        echo "faire le script d'install pureftpd"
     ;;
     no)
        break
     ;;
...
Think to :
  - Configure postfix
  - Configure fail2ban & firewall script
  - Check the vhost
"
